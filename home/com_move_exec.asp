...
<%
	...
	sql = sql & "         , b.credt                                                                                                                "
	sql = sql & "     from  gi_" & menu_type & " b, tree_query c                                                                                   "
	...
	If Not rs.eof Then
		...
		Do Until rs.eof
			...
			sql = sql & "    and credt < (select credt from gi_" & menu_type & " where " & menu_type & "_seq = " & com_seq & ") "
'			msgonly sql
...
'				group_num = clng(rs2("group_min"))
			...
			sql = sql & "    set top_cnt = (select count(*) from gi_" & menu_type & " where menu_seq = '" & old_menu_seq & "' and top_yn = 'Y') "
			sql = sql & "       ,modid = '" & Session("user_id") & "' "
			sql = sql & "       ,moddt = getdate() "
