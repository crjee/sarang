...
'If session("user_id") = "crjee" Then extime("cf_menu ?§Ìñâ?úÍ∞Ñ")
...
<%
	...
	If write_auth <= cafe_mb_level Then ' Í∏Ä?∞Í∏∞ Í∂åÌïú
%>
						<button class="btn btn_c_a btn_s" type="button" onclick="location.href='/home/board_write.asp?menu_seq=<%=menu_seq%>'">Í∏Ä?∞Í∏∞</button>
<%
	End If
%>
						<select id="sch_type" name="sch_type" class="sel w100p">
							<option value="all">?ÑÏ≤¥</option>
							<option value="cb.subject" <%=if3(sch_type="cb.subject","selected","")%>>?úÎ™©</option>
							<option value="cb.agency" <%=if3(sch_type="cb.agency","selected","")%>>Í∏Ä?¥Ïù¥</option>
							<option value="cb.contents" <%=if3(sch_type="cb.contents","selected","")%>>?¥Ïö©</option>
						</select>
						<input type="text" id="sch_word" name="sch_word" value="<%=sch_word%>" class="inp w300p">
						<button type="button" class="btn btn_c_a btn_s" onclick="goSearch()">Í≤Ä??/button>
						</form>
					</div>
					<div class="tb">
						<table>
...
%>
								<tr>
									<td colspan="5" class="td_nodata">µÓ∑œµ» ±€¿Ã æ¯Ω¿¥œ¥Ÿ.</td>
								</tr>
<%
	End If
	rs.close
