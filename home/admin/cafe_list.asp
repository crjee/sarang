...
<%
	...
	If cafe_type = "U" then
		schStr = schStr & " and cafe_type = '" & cafe_type & "' "
	...
	If open_yn <> "" then
		schStr = schStr & " and open_yn = '" & open_yn & "' "
	End If
	...
	If sch_type <> "" And sch_word <> "" then
		If sch_type = "" Then
			schStr = schStr & " and (cf.cafe_name like '%" & sch_word & "%' or cf.cafe_id like '%" & sch_word & "%') "
		Else
				...
				<input type="hidden" name="page" value="<%=page%>">
					<div class="floatL">
						<select name="cafe_type" class="sel w_auto">
							...
							<%=GetMakeCDCombo("cafe_type", cafe_type)%>
						</select>
						<select name="open_yn" class="sel w_auto">
							<option value="">공개여부</option>
							<%=GetMakeCDCombo("open_yn", open_yn)%>
						</select>
...
<script>
	...
	function cafe_find(cafe_id) {
		...
		location.href='cafe_find_exec.asp?cafe_id='+cafe_id
	}
</script>
</html>
