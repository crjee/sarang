...
<%
	...
	If sch_type <> "" And sch_word <> "" Then
		If sch_type = "" Then
			schStr = " where (cc.cafe_name like '%" & sch_word & "%' or mb.agency like '%" & sch_word & "%' or mb.phone like '%" & sch_word & "%' or mb.user_id like '%" & sch_word & "%' or mb.kname like '%" & sch_word & "%') "
		Else
			schStr = " where " & sch_type & " like '%" & sch_word & "%' "
		End If
	Else
		schStr = ""
	End If
...
%>
									...
									<select name="cafe_mb_level_<%=user_id%>" class="sel w_auto" onchange="setColor('<%=i%>')">
										<option value=""></option>
										<%=GetMakeCDCombo("cafe_mb_level", cafe_mb_level)%>
									</select>
									<span id="sp_<%=i%>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
								</td>
								<td class="algC">
					...
					<tbody>
						<tr>
							...
							<td colspan="3">
								...
								<input type="text" id="user_id" name="user_id" class="inp" required>
							</td>
						</tr>
						<tr>
...
<script>
	...
	function goAuth(user_id) {
		try {
			var f = document.search_form;
			f.user_id.value = user_id;
			f.action = "member_root_exec.asp";
			//f.target = "hiddenfrm";
			f.submit();
		}
		catch (e) {
	...
	$(document).ready(function(){
		$('input[name=user_id]').on('keyup keypress',function(){
			alert(this.value);
			//hiddenfrm.location.href = 'member_find_exec.asp?user_id='+user_id
			location.href = 'member_find_exec.asp?user_id='+this.value
		});
	});
</script>
</html>
