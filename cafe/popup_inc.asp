...
%>
	var openwin<%=i%> = window.open('/cafe/popup_view_p.asp?cafe_id=<%=cafe_id%>&popup_key=<%=popup_key%>&menu_seq=<%=menu_seq%>&popup_num=<%=popup_num%>&user_id=<%=session("user_id")%>&ipin=<%=ipin%>', 'open<%=i%>', 'width=450px,height=350px,left=<%=p_left%>,top=<%=p_top%>,resizable=yes');
	setTimeout(function() {
		openwin<%=i%>.focus()
	}, 500);
...
<%
	...
	If Not rs.eof Then
		Do Until rs.eof
			...
			If len(Request.Cookies(popup_key))="0" or Request.Cookies(popup_key)<>"ok" Then
%>
	var opennotice<%=i%> = window.open('/cafe/popup_view_p.asp?notice_seq=<%=notice_seq%>&popup_key=<%=popup_key%>&user_id=<%=session("user_id")%>&ipin=<%=ipin%>', 'notice<%=i%>', 'width=450px,height=350px,left=<%=p_left%>,top=<%=p_top%>,resizable=yes');
	setTimeout(function() {
		opennotice<%=i%>.focus()
	}, 500);
