...
<%
	If tab_use_yn = "Y" Then ' 탭정보 확인
		...
		If section_seq = "" Then section_seq = "0"
...
<%
			Do Until tabRs.eof
%>
						<a href="#n" onclick="goTab('<%=tabRs("section_seq")%>','<%=session("ctTarget")%>')" class="<%=if3(CStr(section_seq)=CStr(tabRs("section_seq")),"on","")%>"><%=tabRs("section_nm")%></a>
<%
				tabRs.MoveNext
			Loop
%>
