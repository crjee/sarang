...
%>
			<div class="container">
<!--#include virtual="/cafe/cafe_banner_inc.asp"-->
...
<%
	...
	Do Until centerRs.eof
		...
		' 홀수 짝수(왼쪽 오른쪽) sub_frm_a : 와이드, sub_frm_l : 2열
		If wide_yn = "Y" Then
			wide_class = "sf_col_1"
		...
		End If
%>
					<div class="<%=odd_even_class%>"><!-- sub_frm_a : 와이드, sub_frm_l : 2열 -->
...
<%
		End If
%>
								<span class="posR"><a href="/cafe/<%=menu_type%>_list.asp?menu_seq=<%=menu_seq%>" target="<%=session("ctTarget")%>">more</a></span>
							</header>
...
<%
		...
		For for_i = 1 To UBound(arrSecSeq)
			...
			If tab_use_yn = "Y" Then ' 탭정보 확인
%>
								<div id="tab_n_cont<%=menu_seq%>_<%=for_i%>" class="tab_cont<%=if3(for_i=1," on","")%>">
<%
			End If
...
<%
				...
				Do Until centerRs2.eof
					...
					view_url = "/cafe/" & menu_type & "_view.asp?" & menu_type & "_seq=" & centerRs2(menu_type & "_seq") & "&menu_seq=" & menu_seq
...
<%
						Else
%>
										<a href="<%=view_url%>" target="<%=session("ctTarget")%>">
											<span class="text">
<%
							If top_yn = "Y" Then
%>
...
%>
										<span class="photos"><a href="<%=view_url%>" target="<%=session("ctTarget")%>"><img src="<%=fileUrl%>" alt="" /></a></span>
<%
						Else
%>
...
<%
						...
						If Not centerRs3.EOF Then
							...
							filePath = thumbnailPath & thmbnl_file_nm
%>
												<span class="photos"><a href="<%=view_url%>" target="<%=session("ctTarget")%>"><img src="<%=fileUrl%>" border="0" /></a></span>
<%
						Else
%>
												<span class="photos"></span>
