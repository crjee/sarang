...
<%
	Else
%>
	$('#cafe_main', parent.parent.document).attr('src', '/cafe/skin/<%=menu_type%>_view.asp?menu_seq=<%=menu_seq%>&page=<%=page%>&sch_type=<%=sch_type%>&sch_word=<%=sch_word%>&<%=menu_type%>_seq=<%=com_seq%>') ;
<%
	End if
%>
</script>
