...
%>
						<button type="button" class="btn btn_c_a btn_s" onclick="<%=session("ctHref")%>location.href='/cafe/skin/waste_notice_list.asp?menu_seq=<%=menu_seq%>'">휴지통</button>
<%
	End If

	If write_auth <= cafe_mb_level Then ' 글쓰기 권한
%>
						<button type="button" class="btn btn_c_a btn_s" onclick="<%=session("ctHref")%>location.href='/cafe/skin/notice_write.asp?menu_seq=<%=menu_seq%>'">글쓰기</button>
<%
	End If
%>
