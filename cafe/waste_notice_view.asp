...
<%
	...
	If link <> "" Then
%>
						<p class="file"><a href="<%=link%>" target="_blink" id="linkTxt"><%=link_txt%></a>&nbsp;<img src="/cafe/img/inc/copy.png" style="cursor:hand" onclick="window.clipnoticeData.setData('Text','<%=link%>');alert('해당 글 주소가 복사되었습니다.\n\n 키보드에 Ctrl + V 누르고 이용하십시요. ')"/></p>
<%
	End If
%>
					</div>
...
<script>
	function goList(gvTarget) {
		document.search_form.action = "/cafe/waste_notice_list.asp";
