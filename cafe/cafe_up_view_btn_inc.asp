<div class="flex-box">
	<div class="item-box">
		<button type="button" class="btn btn_c_n btn_s" onclick="<%=if3(prev_seq="","alert('처음 입니다.')","goPrev()")%>">이전글</button>
		<button type="button" class="btn btn_c_n btn_s" onclick="<%=if3(next_seq="","alert('마지막 입니다')","goNext()")%>">다음글</button>
	...
	<%
		...
		If cafe_mb_level = 10 Or (session("user_id") <> "" And user_id = session("user_id")) Then
	%>
	...
	<%
		...
		If cafe_mb_level = 10 And step_num = "0" Then
	...
	<%
			End If
	%>
		<button type="button" class="btn btn_c_n btn_s" onclick="goTopMove()"><%=if3(top_yn="Y","상위글해제","상위글지정")%></button>
		<script>
			function goMove() {
				lyp('lypp_move');
			}
